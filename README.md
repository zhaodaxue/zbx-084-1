...
### 数据去重（不重复导入）
1. **CSV 内去重**：同文件内完全相同的 `date` + `station_id` + `turbidity_ntu` + `ph` 四列完全相同的记录仅保留第一条，其余标记为重复跳过
2. **数据库级去重**：入库前逐行比对数据库，已存在相同四列完全匹配的记录跳过不重复插入
3. **导入报告**：导入报告显示总行数、成功导入、重复行数、其他跳过行数，并分别显示各类原因和行数

### 页面自动刷新
1. 导入完成后：
   - 自动刷新站点列表
   - 自动选中第一个站点
   - 自动加载该站点的统计面板和日历热力图数据
   - 无需手动切换或重新选站
